import { Department } from '../../../../model/department.class';
import { Observable } from 'rxjs/Observable';
import { HttpClient } from '@angular/common/http';
import { BaseService } from '../../../../core/services/base.service';
import { Injectable } from '@angular/core';
import { environment } from '../../../../../environments/environment';
import { API_URL } from '../../../../shared/constants/api.constant';
import { RequestStatus } from '../../../../model/request-status.class';

@Injectable()
export class RequestStatusService extends BaseService {

  private requestStatus: RequestStatus = new RequestStatus();
  getRequestStatus() {
    return this.requestStatus;
  }
  setRequestStatus(requestStatus) {
    this.requestStatus = requestStatus;
  }

  private findAllUrl = environment.baseUrl + API_URL.FIND_ALL_REQUEST_STATUS;
  private findByTitleUrl = environment.baseUrl + API_URL.REQUEST_STATUS_FIND_BY_TITLE;
  private createOrUpdateUrl = environment.baseUrl + API_URL.REQUEST_STATUS_CREATE_OR_UPDATE;

  constructor(httpClient: HttpClient) {
    super(httpClient);
  }

  findAll(params: {}): Observable<any> {
    return this.get(this.findAllUrl, params);
  }

  findByTitle(title): Observable<any> {
    return this.get(this.findByTitleUrl, title);
  }

  createOrUpdate(params: {}): Observable<any> {
    return this.post(this.createOrUpdateUrl, params);
  }

}
