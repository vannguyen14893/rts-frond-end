...
import {
  denormalizeCandidate, denormalizeRequest, denormalizeCandidateStatus,
  denormalizeInterview, denormalizeUser, denormalizeCandidateArray,
  denormalizeInterviewArray, denormalizeLogArray, denormalizeCandidateStatusArray,
...
export class StoredProcedureService {
  ...
  filterCandidates() {
    ...
    if (currentRequestAssigneeId === 0) {
      ...
      candidateIdsByAssignee = currentRequestCandidateIds
        .filter(id => allCandidates[id].createdBy === currentRequestAssigneeId);
    }
